...
export async function POST(request: Request) {
  try {
    ...
    if (!apiUrl || !apiKey) {
      console.error('[estimate] PTR Lead API env vars not configured.');
      return NextResponse.json(
    ...
    try {
      const apiResponse = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          smsConsentPromo: data.smsConsentPromo === true,
          smsConsentAt: data.smsConsent === true ? new Date().toISOString() : undefined,
      ...
      if (!apiResponse.ok) {
        ...
        console.error('[estimate] PTR Lead API error:', apiResponse.status, detail);
        return NextResponse.json(
