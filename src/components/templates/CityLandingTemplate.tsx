...
import { getCityStateSlug } from '@/lib/locations';
import { getServiceBySlug } from '@/lib/services';
import { CITY_SERVICE_SLUGS } from '@/lib/city-services';
...
export default function CityLandingTemplate({
  ...
  const serviceCards = CITY_SERVICE_SLUGS.map((slug) => {
    const svc = getServiceBySlug(slug);
    return svc
      ? { name: svc.name, slug: svc.slug, shortDescription: svc.shortDescription }
      : null;
  }).filter(Boolean) as {
    name: string;
    slug: string;
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {serviceCards.map((svc) => (
              <Link
                key={svc.slug}
                href={`/locations/${cityStateSlug}/${svc.slug}`}
              ...
              >
                ...
                <p className="mt-2 text-neutral-600">{svc.shortDescription}</p>
